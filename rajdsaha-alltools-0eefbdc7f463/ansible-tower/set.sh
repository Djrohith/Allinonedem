...
if [ "$1" = "ansible-tower" ]; then
	...
	sleep 20
        ...
        tower-cli project create --name source --scm-type git --scm-url="https://bitbucket.org/rajdsaha/kubernates.git" --scm-branch master --scm-credential test --scm-update-on-launch true
        tower-cli credential create --name deploy-key --organization="Default" --kind ssh --username ec2-user --ssh-key-data /opt/keypair
	tower-cli inventory create --name deploy --organization="Default"
	tower-cli group create --name master --inventory deploy
        whoami
	tower-cli host create --name $(wget -q -O - http://169.254.169.254/latest/meta-data/local-ipv4) --inventory deploy
        tower-cli host associate --host $(wget -q -O - http://169.254.169.254/latest/meta-data/local-ipv4) --group master
        sleep 30
        tower-cli job_template create --name petclinic --job-type run --inventory deploy --project source --playbook deploy.yml --machine-credential deploy-key --extra-vars "TAG=1" --extra-vars "REGISTRY=localhost" --ask-variables-on-launch true
        sleep inf & wait
else
	exec "$@"
fi
