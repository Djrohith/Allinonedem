...
if [ "$1" = "ansible-tower" ]; then
	...
	ansible-tower-service start
        whoami
	sleep 20
        tower-cli credential create --name test --organization="Default" --kind scm --username rohitreddy.gunna@gapgemini.com --password Omsairam09
	  	tower-cli project create --name source --scm-type git --scm-url="https://rohithreddy09@bitbucket.org/rohithreddy09/alltoolsinkubernets.git" --scm-branch master --scm-credential test --scm-update-on-launch true
	...
	tower-cli group create --name master --inventory deploy
        whoami
	tower-cli host create --name masterip1 --inventory deploy
        tower-cli host associate --host masterip1 --group master
		
		
	tower-cli host create --name masterip2 --inventory deploy
        tower-cli host associate --host masterip2 --group master
		
	tower-cli host create --name masterip3 --inventory deploy
        tower-cli host associate --host masterip3 --group master
	    sleep 30
