...
sed -i s/masterip1/$(kubectl get nodes | grep master | awk {'print $1'} | grep -o -P '(?<=ip-).*$' | awk -F . {'print $1'}| sed s/-/./g | sed -n '1p')/g ansible-tower/set.sh
sed -i s/masterip2/$(kubectl get nodes | grep master | awk {'print $1'} | grep -o -P '(?<=ip-).*$' | awk -F . {'print $1'}| sed s/-/./g | sed -n '2p')/g ansible-tower/set.sh
sed -i s/masterip3/$(kubectl get nodes | grep master | awk {'print $1'} | grep -o -P '(?<=ip-).*$' | awk -F . {'print $1'}| sed s/-/./g | sed -n '3p')/g ansible-tower/set.sh
